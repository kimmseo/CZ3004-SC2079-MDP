...
turn_wrt_big_turns = [[4 * Constants.TURN_RADIUS, 1 * Constants.TURN_RADIUS],
                  [4 * Constants.TURN_RADIUS, 1 * Constants.TURN_RADIUS]]
...
class Navigator:
    ...
    @staticmethod
    def compute_state_distance(start_state: Cell, end_state: Cell, level=1):
        """Compute the L-n distance between two cell states

        Args:
            start_state (Cell): Start cell state
            end_state (Cell): End cell state
            level (int, optional): L-n distance to compute. Defaults to 1.

        Returns:
            float: L-n distance between the two given cell states
        """
        return Navigator.compute_coord_distance(start_state.x, start_state.y, end_state.x, end_state.y, level)
    ...
    def get_optimal_order_dp(self, retrying = False) -> List[Cell]:
        distance = 1e9
        optimal_path = []
        ...
        for op in self.get_visit_options(len(all_view_positions)):
            ...
            combination = []
            self.generate_combination(cur_view_positions, 0, [], combination, [Constants.ITERATIONS])
            ...
            for c in combination: # run the algo some times ->
                ...
                for i in range(len(_permutation) - 1):
                    ...
                    for j in range(1, len(cur_path)):
                        optimal_path.append(Cell(cur_path[j][0], cur_path[j][1], cur_path[j][2]))
    ...
    def generate_combination(view_positions, index, current, result, iteration_left):
        ...
        for j in range(len(view_positions[index])):
            current.append(j)
            Navigator.generate_combination(view_positions, index + 1, current, result, iteration_left)
            current.pop()
    ...
    def get_neighbors(self, x, y, direction):  # TODO: see the behavior of the robot and adjust...
        ...
        for dx, dy, md in Constants.MOVE_DIRECTION:
    ...
    def path_cost_generator(self, states: List[Cell]):
        """Generate the path cost between the input states and update the tables accordingly

        Args:
            states (List[Cell]): cell states to visit
        """
        ...
        def astar_search(start: Cell, end: Cell):
            ...
            while heap:
                ...
                for next_x, next_y, new_direction, safe_cost in self.get_neighbors(cur_x, cur_y, cur_direction):
                    ...
                    move_cost = Direction.rotation_cost(new_direction, cur_direction) * Constants.TURN_FACTOR + 1 + safe_cost
