            ...
            if not bt_queue.empty():
                ...
                if command == "STM32":
                    ...
                    if optional_parts:  # This checks if there are any optional parts
                        ...
                        msg_to_android = f'{{"cat":"image-rec", "value":{{"obstacle_id":{number_part}, "image_id":{predict_id}}}}}'
                    ...
                    print(f"FIN msg: {msg}\n")
                    msg_to_android = '{"cat":"status", "value":"finished"}'
                    ...
                    if commands:
                        for command, associated_path in command_path:
                                ...
                                if command.startswith(substring):
                                    ...
                                    if associated_path:
                                        bt_queue.put_nowait(("STM32", command, associated_path))
                                    algo_start_event.clear()
                                    stm_start_event.set()

                                elif command == "FIN":
                                    bt_queue.put_nowait((command,))
                                    # bt_start_event.set()
    ...
    if status_response and status_response.get('status') == 'OK':
        ...
        while True:
            if not ir_queue.empty():
                ...
                bt_queue.put_nowait(("IR_CAPTURING",))
                file_path = take_pic()
                ...
                bt_queue.put_nowait(("IR_TARGET", number_part, predict_id, direction))
...
def stmSendProcess(STM, stm32_send_queue, stm32_recv_queue, stm_start_event):
    ...
    while True:
        if not stm32_send_queue.empty():
            ...
            STM.send(command)
            stm32_recv_queue.put_nowait((command, associated_path))
...
def stmRecvProcess(STM, stm32_recv_queue, bt_queue, bt_start_event, algo_start_event):
    while True:
        if not stm32_recv_queue.empty():
            msg = stm32_recv_queue.get_nowait()
            ...
            while(True):
                ...
                if received_msg=="R":
                    ...
                    print("Timeout waiting for RPI to receive message")
                    break
