...
def startBTServer(bt_queue, cmd_queue, ir_queue, stm32_send_queue, stm32_recv_queue,
    ...
    try:
        ...
        time.sleep(10)
        msg = "FIN"
        ir_queue.put_nowait(msg)
        stm32_send_queue.put_nowait(msg)

        cmd_start_event.set()
        ir_start_event.set()
        stm_start_event.set()
        stm32_recv_queue.put_nowait(msg)
...
def cmdGeneratorTemp(bt_queue, cmd_queue, ir_queue, stm32_send_queue,
    ...
    while True:
        ...
        while True:
            if not cmd_queue.empty():
                predict_id = cmd_queue.get_nowait()
                if predict_id == "38":
                    direction = "R"
                if predict_id == "-1":
        ...
        #             'FR060','FL060','US125']
        commands = ['AS020','AS020',
                           'UL050','AS020', 'UR050', 'AS020'
                           'UR060', "AS020",'UL060','AS020']
        if direction == "R":
            commands = ['AS020','AS020',
                        'UR050','AS020','UL050','AS020',
                        'UL060','AS011','UR060','AS020']
            # commands =   ['FR045','FW020','FL045','FW010',
        ...
        # assuming:
            # turn to face side and move to edge
            # turn corner to go behind
            # move to other edge 
            # turn corner to go back to front
            # move forward towards carpark estimated distance travelled foward
            # turn into carpark based on whichever side the robot is on (left or right side)
        # Default left commands
        commands = ['AS020', 'OL090']
        # commands = ['FL090','FW018',
        #             'FR090','FW010','FR090','FW087','FR090',
        #             'RT040','FW000','FR090','FW023','FL090','US015']
        if direction == "R":
            commands = ['AS020', 'OR090']
    ...
    if status_response and status_response.get('status') == 'OK':
        ...
        while True:
            if not ir_queue.empty():
                ...
                cmd_queue.put_nowait(predict_id)
                cmd_start_event.set()
